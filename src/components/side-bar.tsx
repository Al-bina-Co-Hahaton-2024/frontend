...
export const SideBar = () => {
    ...
    return(
        <div className={'w-[150px] h-full bg-white rounded-tl-[0px] rounded-tr-[20px] rounded-br-[20px] p-4 flex flex-col justify-between items-center custom-sidebar-shadow'}>
            <img src={logo} alt={'logo'}/>
            <div className={'flex flex-col gap-[10px]'}>
                {
                    localStorage.getItem('role') === 'hr' &&  <Link to={'/employees'} className={`${location.pathname.includes('employees') ? 'bg-[#E5F6FF]': 'bg-white'} p-2  rounded-[10px]`}>
