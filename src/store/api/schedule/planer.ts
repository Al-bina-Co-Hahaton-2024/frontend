...
export const planerApi = baseApi.injectEndpoints({
    endpoints: (builder) => ({
        ...
        getWeekNums: builder.query({
            query: (dates: string[]) => `/reference/week-numbers?dates=${dates.join(',')}`
        }),
...
export const {useGetAnalyzesWeeksMutation, useGetGraphOnMonthQuery, useGetWeekNumsQuery,  useLazyGetGraphOnMonthQuery, useLazyGetWeekNumsQuery} = planerApi